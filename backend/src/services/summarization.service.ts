...
export const summarizeContent = async (content: string): Promise<string> => {
    ...
    while (attempt < MAX_RETRIES) {
        try {
            ...
            const result = await chatSession.sendMessage(`Summarize the following article:\n\n${content}`);
            ...
            if (error.status === 429 && attempt < MAX_RETRIES) {
                console.warn(`Summarization rate limited. Retrying attempt ${attempt} after ${RETRY_DELAY_MS}ms...`);
