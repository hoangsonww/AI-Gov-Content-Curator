...
export function tryLoadOrchestrationEnv(
  ...
  | { success: true; data: OrchestrationEnv }
 ...
 */
export function preflightCheck(env: OrchestrationEnv): {
  ready: boolean;
  providers: string[];
  warnings: string[];
} {
  ...
  if (env.ANTHROPIC_API_KEY) {
    ...
    warnings.push(
      "ANTHROPIC_API_KEY not set — Anthropic agents will be unavailable",
    );
  }
  ...
  if (providers.length === 0) {
    warnings.push(
      "CRITICAL: No LLM provider configured — all generation calls will fail",
    );
  }
