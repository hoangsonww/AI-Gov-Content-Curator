...
def _value(metric, **labels) -> float:
    ...
    if labels:
        sample = metric.labels(**labels)._value.get()  # noqa: SLF001
    else:
        sample = metric._value.get()  # noqa: SLF001
    return float(sample)
...
def test_metrics_singleton(fresh_metrics_registry) -> None:  # noqa: ARG001
    m1 = metrics()
    m2 = metrics()
    assert m1 is m2
...
def test_record_llm_call_increments(fresh_metrics_registry) -> None:
    ...
    assert _value(m.llm_tokens_total, provider="openai", model="gpt-4o-mini", direction="prompt") == 100.0
    assert _value(m.llm_tokens_total, provider="openai", model="gpt-4o-mini", direction="completion") == 200.0


def test_traced_span_sync(fresh_metrics_registry) -> None:  # noqa: ARG001
    with traced_span("test.span", foo="bar") as span:
        assert span is not None


@pytest.mark.asyncio
async def test_traced_async_span(fresh_metrics_registry) -> None:  # noqa: ARG001
    async with traced_async_span("test.async") as span:
        assert span is not None


@pytest.mark.asyncio
async def test_traced_decorator_async(fresh_metrics_registry) -> None:  # noqa: ARG001
    @traced("decorated")
...
def test_traced_decorator_sync(fresh_metrics_registry) -> None:  # noqa: ARG001
